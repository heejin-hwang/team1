...
function judgeBtn() {
   ...
   /*  if(id_check == 1 && name_check == 1 && mobile_phone_check == 1 && address_check == 1){ */
	...
	$(function(){ 
		...
		<div class="table">
			<form id="Form" method="POST" action="/Student/editStudent?target_id=${detail.id}">
				<table id="register" class="boardtable">
					...
					<tr>
						<th>이름</th>
						<td><input type="hidden" id="name" name="name" value ="${detail.name}">${detail.name}</td>
					</tr>
					...
					<tr>
						...
						<td><input id="address" name="address" class="address" type="text" maxlength="11" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"
...
<body>
	...
	<div class="section">
		...
		<div class="table">
			<form id="Form" method="POST" action="/Student/editStudent">
				<table id="register">
					<input id ="targetId" name = "targetId" value = "${targetId}" type>
					<tr>
						<th>학생아이디 <span class="required">*</span></th>
					...
					<tr>
						<th>훈련과정<span class="required">*</span></th>
						<td><select name="course_code" id="course_code">
								<option value="7" selected="selected">제7차
									프론트엔드(ReactJS, AngularJS) 개발을 위한 자바(JAVA) 웹개발자 양성과정</option>
								<option value="6">제6차 프론트엔드(ReactJS, AngularJS) 개발을 위한
									자바(JAVA) 웹개발자 양성과정</option>
								<option value="5">제5차 프론트엔드(ReactJS, AngularJS) 개발을 위한
									자바(JAVA) 웹개발자 양성과정</option>
								<option value="1">제4차 프론트엔드(ReactJS, AngularJS) 개발을 위한
									자바(JAVA) 웹개발자 양성과정</option>
						</select></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><select name="state" id="state">
								<option value="now" selected="selected">훈련중</option>
								<option value="end">훈련종료</option>
								<option value="drop">중도하차</option>
						</select></td>
					</tr>
					<tr>
						<th>출석일수</th>
					...
					<tr>
						...
						<td><input type="tel" id="mobile_phone" name="mobile_phone"
							required value="${detail.mobile_phone}"></td>
					</tr>
					<tr>
						<th>비상연락처</th>
						<td><input type="tel" id="temporarily_number" name="temporarily_number"
					...
					<tr>
						<th>기본주소</th>
						<td><input name="address" id="address" class="address" type="text"
							value="${detail.address}"></td>
					</tr>
					<tr>
						<th>상세주소</th>
						<td><input name="detail_address" id="detail_address" class="address" type="text"
							value="${target.detail_address}"></td>
					</tr>
				...
				<table id="register_button">
					<div class="button">
						...
						<input type="button" value="취소"
							onclick="location.href='detailStudent';">
					</div>
				</table>
