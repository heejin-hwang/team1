        ...
        <div class="search">
           <form id="form">
            ...
            <div class="select_C_N">
             <!---------------반 목록 select박스--------------->
            <c:if test="${login.job_id eq '0' || login.job_id eq '1' || login.job_id eq '2' }">
        	...
        	</form> <!-- end form ::: action : Notice // addBtn-> action : addNotice (jquery) -->
        </div>      <!-- end div ::  search -->
               ...
               <c:if test = "${pageMaker.pot }">
				...
				<tr class="table_list">
                    ...
                    <td class="list_subject">
                    	<a href="detailNotice?id=${list.id}&scourse_code=${cri.scourse_code }&scategorize=${cri.scategorize}&cstate=${cri.cstate}&searchType=${cri.searchType}&keyword=${cri.keyword}&page=${cri.page}">
                    					${list.subject}
                    	</a>
                    </td>
                    <td>${list.name}</td>
            ...
            <div class="center">
            	<div class="pagination">
					<c:if test="${pageMaker.pprev}">
			  			<a href="Notice${pageMaker.makeSearch(pageMaker.startPage-1) }">&laquo;</a>
			  		</c:if>
			  		...
			  		<c:if test="${pageMaker.nnext && pageMaker.endPage > 0 }">
			  			<a href="Notice${pageMaker.makeSearch(pageMaker.endPage+1) }">&raquo;</a>
			  		</c:if>
